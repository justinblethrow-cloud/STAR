...
\begin{optOptTable}
  ...
  \optOpt{genomeInsert}   \optOptLine{add named FASTA sequences to an existing genome index and write a complete updated index, overlay, or cached delta directory}
...
\optName{genomeDir}
  \optValue{./GenomeDir/}
  \optLine{string: path to the directory where genome files are stored (for --runMode alignReads and --runMode genomeInsert) or will be generated (for --runMode generateGenome)}
\optName{genomeLoad}
...
\optName{genomeFastaFiles}
  ...
  \optLine{Required for the genome generation (--runMode genomeGenerate). Can also be used in the mapping (--runMode alignReads) to add extra (new) sequences to the genome (e.g. spike-ins), or with --runMode genomeInsert to persistently add named sequences to an existing genome index.}
\optName{genomeInsertOutDir}
  \optValue{-}
  \optLine{string: path to the output genome directory for --runMode genomeInsert. The input --genomeDir is read but not modified; the contents written to --genomeInsertOutDir are controlled by --genomeInsertOutMode. Use --sjdbGTFfile to add annotations for inserted sequences only.}
\optName{genomeInsertOutMode}
  \optValue{Full}
  \optLine{string: output mode for --runMode genomeInsert.}
\begin{optOptTable}
  \optOpt{Full}   \optOptLine{write a complete updated genome index to --genomeInsertOutDir}
  \optOpt{Overlay}   \optOptLine{write a small integrity-checked package containing the inserted FASTA/GTF and a reference to the content-identified base --genomeDir. The package can be used as --genomeDir with --runMode alignReads}
  \optOpt{Delta}   \optOptLine{write an Overlay package plus a validated cached suffix insertion plan. The package can be used as --genomeDir with --runMode alignReads to avoid repeating suffix search and sorting for the inserted FASTA}
\end{optOptTable}
...
\optName{sjdbGTFfile}
  \optValue{-}
  \optLine{string: path to the GTF file with annotations. With --runMode genomeInsert, this GTF must contain annotations for inserted sequences only.}
...
\optName{readChunkSizeBytes}
  \optValue{0}
  \optLine{int{\textgreater}=0: target input chunk buffer size (bytes), total across mates, per thread. 0 uses 1000000 bytes for {\textgreater}=64 mapping threads and limitIObufferSize otherwise}
