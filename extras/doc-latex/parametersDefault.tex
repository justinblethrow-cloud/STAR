...
\optName{genomeInsertOutDir}
  \optValue{-}
  \optLine{string: path to the output genome directory for --runMode genomeInsert. The input --genomeDir is read but not modified; a complete updated genome index is written to --genomeInsertOutDir. Adding new GTF annotations for inserted sequences is not supported in this mode.}
