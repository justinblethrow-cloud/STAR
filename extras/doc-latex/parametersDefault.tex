...
\begin{optOptTable}
  ...
  \optOpt{genomeInsert}   \optOptLine{add named FASTA sequences to an existing genome index and write a complete updated index to a new directory}
...
\optName{genomeInsertOutDir}
  \optValue{-}
  \optLine{string: path to the output genome directory for --runMode genomeInsert. The input --genomeDir is read but not modified; a complete updated genome index is written to --genomeInsertOutDir. Use --sjdbGTFfile to add annotations for inserted sequences only.}
