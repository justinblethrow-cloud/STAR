...
\begin{optOptTable}
  ...
  \optOpt{Overlay}   \optOptLine{write a small overlay directory that refers to the base --genomeDir and inserted FASTA/GTF files. The overlay can be used as --genomeDir with --runMode alignReads}
  \optOpt{Delta}   \optOptLine{write an overlay directory plus a cached suffix insertion plan. The delta can be used as --genomeDir with --runMode alignReads to avoid repeating suffix search and sorting for the inserted FASTA}
\end{optOptTable}
